...
public class Pawn extends ChessPiece{
	...
	public boolean[][] possibleMoves() {
		...
		if(getColor() == Color.WHITE) {//white pawn -> going up
			...
			p.setValues(position.getRow() - 1, position.getColumn() - 1); //is there a piece left up?
			if(getBoard().positionExists(p) && isThereOpponentPiece(p)) {
				mat[p.getRow()][p.getColumn()] = true;
			}
			p.setValues(position.getRow() - 1, position.getColumn() + 1); //is there a piece right up?
			if(getBoard().positionExists(p) && isThereOpponentPiece(p)) {
				mat[p.getRow()][p.getColumn()] = true;
			}
			...
			if(position.getRow() == 3) {//only can happen in row 5 
			...
			if(position.getRow() == 4) {//only can happen in row 5 
				Position left = new Position(position.getRow(), position.getColumn() - 1);
				if(getBoard().positionExists(left) && isThereOpponentPiece(left) && getBoard().piece(left) == chessMatch.getEnPassantVunerable()) {
					mat[left.getRow() + 1][left.getColumn()] = true;
				}
